...
tags: ["query:llm-cp"]
score: 6.0
evidence: 通过解码调度器实现扩散语言模型的快速一致推理
...
## 速览
**TLDR**：扩散语言模型（DLM）虽具备并行生成潜力，但传统“分散接受”解码方式导致KV缓存碎片化和推理效率低下。本文提出LSP调度器，这是一种无需训练且模型无关的推理范式。它通过识别最长稳定前缀并结合自然语言定界符进行原子提交，将碎片化更新转为连续追加，显著减少了令牌翻转和去噪次数。在LLaDA-8B等模型上实验显示，LSP在保持质量的同时实现了高达3.4倍的推理加速。 \
**Motivation**：传统的DLM解码采用分散接受方式，导致KV缓存碎片化、内存局部性差以及频繁的令牌修复，严重限制了实际推理速度。 \
**Method**：提出LSP调度器，在每步去噪中动态识别左对齐的连续稳定令牌块，并将其边界对齐至自然语言定界符后进行一次性提交。 \
**Result**：在数学推理、代码生成和多语言任务中，LSP相比现有方法实现了高达3.4倍的加速，且输出质量持平或略有提升。 \
**Conclusion**：LSP通过重构令牌提交拓扑结构，有效弥合了DLM理论并行性与实际硬件效率之间的差距，显著提升了推理的连贯性与效率。
