...
## 摘要
现代语言模型仍然依赖于固定的、预定义的子词分词。一旦分词器训练完成，语言模型就只能在这一固定的粒度水平上运行，这往往会导致即使在性能强大的推理模型中也会出现脆弱且违反直觉的行为。我们提出了 ByteFlow Net，这是一种全新的层次化架构，它完全去除了分词器，转而使模型能够学习将原始字节流分割成具有语义意义的单元。ByteFlow Net 基于潜在表示的编码率执行压缩驱动的分段，在通过 Top-K 选择保持静态计算图的同时，产生自适应边界。与以往依赖于带有认为设计归纳偏置的脆弱启发式方法的自分词方法不同，ByteFlow Net 能够根据输入本身调整其内部表示的粒度。实验表明，这种基于压缩的分块策略带来了显著的性能提升，ByteFlow Net 的表现优于基于 BPE 的 Transformer 以及之前的字节级架构。这些结果表明，端到端的无分词器建模不仅可行，而且更加有效，为开发更具自适应性和信息基础的语言模型开辟了道路。
