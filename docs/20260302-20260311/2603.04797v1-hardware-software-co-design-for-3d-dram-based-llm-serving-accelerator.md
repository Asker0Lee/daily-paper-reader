...
## 速览
**TLDR**：本研究针对大语言模型（LLM）在线服务中动态负载和KV缓存管理效率低的问题，提出了基于3D-DRAM的软硬件协同设计加速器Helios。Helios利用混合键合技术，设计了分布式分块注意力执行流与空间感知KV缓存分配机制，解决了近存计算（NMP）在处理非均匀内存抽象时的瓶颈。实验表明，Helios在性能和能效上显著优于现有GPU和NMP方案，大幅降低了推理延迟。 \
**Motivation**：现有的近存计算设计因KV缓存管理粗粒度且执行流不灵活，难以高效应对LLM服务中高度动态的负载需求。 \
**Method**：提出Helios加速器，采用分布式分块注意力执行流、跨处理单元通信原语及空间感知KV缓存分配机制。 \
**Result**：实验结果显示，Helios相比现有GPU/NMP设计实现了3.25倍的加速和3.36倍的能效提升，并显著改善了长尾延迟。 \
**Conclusion**：Helios通过软硬件协同优化，成功弥合了动态KV缓存管理与分布式近存计算架构之间的鸿沟，提升了LLM推理效率。
