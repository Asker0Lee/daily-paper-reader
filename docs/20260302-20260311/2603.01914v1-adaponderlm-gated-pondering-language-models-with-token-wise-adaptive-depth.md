...
tags: ["query:llm-cp"]
score: 8.0
evidence: 用于高效推理的标记级自适应深度
tldr: "AdaPonderLM 针对循环 Transformer 在推理时对简单 token 浪费计算资源的问题，提出了一种自监督的 token 级自适应深度模型。它在预训练阶段通过 MLP 门控机制和单调停止掩码学习早期退出策略，并引入 KV 重用机制确保训练推理一致性。实验表明，该方法在保持性能的同时减少了约 10% 的计算量，实现了对困难 token 的动态资源分配。"
selection_source: fresh_fetch
motivation: 现有的循环语言模型通常运行固定次数的迭代，无法根据 token 的难易程度动态调整计算量，导致推理效率低下。
method: 提出 AdaPonderLM，利用迭代特定的 MLP 门控和单调停止掩码实现 token 级早期退出，并结合 KV 重用机制加速推理。
result: "在不同规模的 Pythia 模型上，AdaPonderLM 在保持语言建模性能和下游任务准确率的同时，减少了约 10% 的推理计算量。"
conclusion: 该模型通过自监督学习实现了有效的自适应计算分配，证明了在循环架构中针对性分配计算资源优于固定剪枝策略。
---
...
**Method**：提出 AdaPonderLM，利用迭代特定的 MLP 门控和单调停止掩码实现 token 级早期退出，并结合 KV 重用机制加速推理。 \
**Result**：在不同规模的 Pythia 模型上，AdaPonderLM 在保持语言建模性能和下游任务准确率的同时，减少了约 10% 的推理计算量。 \
**Conclusion**：该模型通过自监督学习实现了有效的自适应计算分配，证明了在相同计算预算下，动态停止策略优于固定剪枝。
