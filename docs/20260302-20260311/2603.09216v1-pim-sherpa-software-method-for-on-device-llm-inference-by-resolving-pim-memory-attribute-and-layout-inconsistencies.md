...
title_zh: PIM-SHERPA：通过解决 PIM 内存属性与布局不一致性实现端侧 LLM 推理的软件方法
...
tags: ["query:llm-cp"]
score: 7.0
evidence: 通过内存属性和布局管理优化端侧LLM推理
tldr: "针对端侧大模型推理中预填充和解码阶段对内存属性及布局需求不一致的问题，本文提出 PIM-SHERPA 软件方案。通过 DRAM 双缓冲（DDB）和在线权重重排（OWR）两种技术，解决了缓存一致性与 PIM 布局冲突，在 Llama 3.2 模型上实现了约 48% 的内存节省，且性能接近理论峰值，是首个在产品级 PIM 系统上解决此类问题的研究。"
...
## 摘要
大语言模型（LLM）的端侧部署正在移动和边缘平台迅速普及。LLM 推理包含计算密集型的预填充（prefill）阶段和内存带宽密集型的解码（decode）阶段，学术界和工业界普遍认为解码阶段非常适合存内计算（PIM）。然而，实际的 PIM 系统在这两个阶段之间面临两个障碍：一是内存属性不一致，即预填充阶段倾向于将权重放置在可缓存区域以供复用，而解码阶段则需要权重位于不可缓存区域以可靠地触发 PIM；二是主机友好型布局与 PIM 感知型布局之间的权重布局不一致。为了解决这些问题，我们提出了 PIM-SHERPA，这是一种通过解决 PIM 内存属性和布局不一致性来实现高效端侧 LLM 推理的纯软件方法。PIM-SHERPA 提供了两种方案：DRAM 双缓冲（DDB），它将单一的 PIM 感知权重保留在不可缓存区域，同时将下一层的交织（swizzled）权重预取到小型可缓存缓冲区中；以及带有交织内存拷贝的在线权重重排（OWR），它在 GEMM 执行前立即进行按需的交织内存拷贝。与基准 PIM 仿真系统相比，PIM-SHERPA 在 Llama 3.2 模型上实现了约 47.8% - 49.7% 的内存容量节省，同时保持了与理论最大值相当的性能。据我们所知，这是首个识别出内存属性不一致性并在产品级 PIM 系统上提出有效解决方案的工作。
