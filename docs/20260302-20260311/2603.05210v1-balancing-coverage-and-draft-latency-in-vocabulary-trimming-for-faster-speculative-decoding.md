...
## 速览
**TLDR**：本研究针对投机解码中草稿模型因词表过大导致推理延迟高的问题，提出了草稿词表精简（Vocabulary Trimming）方法。该方法将词表选择建模为平衡令牌覆盖率与架构感知延迟的约束优化问题，并利用 Tree-structured Parzen Estimator 寻找最优 Pareto 前沿。实验表明，该方法能将草稿词表缩减高达 97%，在特定领域任务中提升 20% 的吞吐量，在分布外任务中也有显著增益，有效优化了投机解码的效率。 \
**Motivation**：投机解码中草稿模型的语言模型头因词表过大而产生高昂的计算成本，成为限制推理速度的瓶颈。 \
**Method**：将词表选择视为受限优化问题，通过 TPE 算法在覆盖率和基于 FLOPs 的延迟估算之间寻找最佳平衡点。 \
**Result**：在保持高覆盖率的同时，成功缩减草稿词表达 97%，使特定领域任务的吞吐量提升了 20%。 \
**Conclusion**：通过精简草稿模型词表，可以有效解决覆盖率与延迟之间的权衡，显著提升大语言模型的投机解码效率。
