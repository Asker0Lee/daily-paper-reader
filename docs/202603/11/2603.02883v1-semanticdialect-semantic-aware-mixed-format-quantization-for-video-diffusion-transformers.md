...
motivation: 视频DiT模型计算开销巨大，且现有量化方法在处理高激活波动和维持视频语义连贯性方面表现不佳。
method: 提出一种基于查找表的块级混合格式量化方案，并结合注意力引导的残差补偿和语义相关的格式共享机制。
result: 在VDiT模型上的实验表明，该方法显著优于现有量化基准，在Open-Sora 2.0上实现了接近无损的量化效果。
conclusion: SemanticDialect通过精细化的格式选择和语义一致性优化，为高性能视频生成模型的低比特部署提供了有效路径。
---
