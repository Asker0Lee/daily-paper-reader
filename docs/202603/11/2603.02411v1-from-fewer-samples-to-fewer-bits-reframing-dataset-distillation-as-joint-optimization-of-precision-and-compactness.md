...
## 摘要
数据集蒸馏 (DD) 将大型数据集压缩为紧凑的合成数据集，并保持训练性能。然而，当前的方法主要侧重于样本缩减，较少考虑数据精度及其对效率的影响。我们提出了量化感知数据集蒸馏 (QuADD)，这是一个在固定比特预算下联合优化数据集紧凑性与精度的统一框架。QuADD 在蒸馏循环中集成了一个可微分量化模块，实现了合成样本与量化参数的端到端协同优化。在率失真视角的指导下，我们实证分析了样本数量与精度之间的比特分配如何影响学习性能。我们的框架支持均匀量化和自适应非均匀量化，其中后者能从数据中学习量化能级，以更好地表示信息密集区域。在图像分类和 3GPP 波束管理任务上的实验表明，QuADD 在单位比特准确率上超越了现有的 DD 和后量化基准，为信息高效的数据集蒸馏建立了新标准。
